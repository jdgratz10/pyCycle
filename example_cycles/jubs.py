...
if __name__ == "__main__":
    ...
    ##Issue: we could connect the areas in the design case to the areas in the off design case. This works when running a des case right before
    ##an od case. Or, before we created an indepVarComp and set the value in the indepVarComp equal to the area value we wanted. Now, however,
    ##the problem is that with auto ivc, no connection is made if we run an off design case without running a design case, and we just set the 
    ##value instead. This is where we run into issues.