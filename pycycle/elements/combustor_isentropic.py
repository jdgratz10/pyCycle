...
class IsentropicCombustor(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem('mix_fuel',
                           MixFuel(),
                           promotes=['Fl_I:stat:W','Fl_I:FAR', 'Fl_I:tot:h', 'Wfuel', 'Wout'])
        ...
        vit_flow = SetTotal(mode='h', fl_name="Fl_O:tot", gamma=gamma, S_data=S_data, h_base=h_base, T_base=T_base, Cp=Cp, MW=air_fuel_MW)
        ...
        if statics:
            if design:
                ...
                out_stat = SetStatic(mode="MN", fl_name="Fl_O:stat", computation_mode='isentropic', thermo_data=janaf, gamma=gamma, S_data=S_data, h_base=h_base, T_base=T_base, Cp=Cp, MW=air_fuel_MW)
                prom_in = ['MN']
                prom_out = ['Fl_O:stat:*']
                ...
                # Calculate static properties.
                out_stat = SetStatic(mode="area", fl_name="Fl_O:stat", computation_mode='isentropic', thermo_data=janaf, gamma=gamma, S_data=S_data, h_base=h_base, T_base=T_base, Cp=Cp, MW=air_fuel_MW)
                prom_in = ['area']
                prom_out = ['Fl_O:stat:*']
                self.add_subsystem('out_stat', out_stat, promotes_inputs=prom_in,
                                   promotes_outputs=prom_out)
